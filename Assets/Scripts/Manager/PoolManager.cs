...
public class PoolManager : MonoBehaviour
{
    ...
    public static T SpawnFromPool<T>(string tag, Vector2 position)
    {
    ...
    public static T SpawnFromPool<T>(string tag, Vector2 position, Quaternion rotation)
    {
    ...
    private void Start()
    {
        ...
        poolDictionary = new Dictionary<string, Queue<GameObject>>(); //껍대기
    ...
    GameObject CreateNewObject(string tag, GameObject prefab)
    {
        ...
        obj.name = tag;
        obj.SetActive(false); //비활성시 ReturnToPool을 하므로 Enqueue가 됨
        return obj;
    }
